...
kotlin {
  ...
  androidTarget {
    ...
    android {
      ...
      sourceSets { getByName("main") { jniLibs.srcDirs(jniBasePath().absolutePath) } }
    }
  ...
  sourceSets {
    ...
    jvmMain { sourceSets { resources.srcDirs(jvmLibResourcesDirectory.absolutePath) } }
...
configureDownloadTasks {
  ...
  download("wgpu-macos-aarch64-release.zip") {
    extract("wgpu-native-meta/webgpu.yml", buildNativeResourcesDirectory.resolve("webgpu.yml"))
      .doLast {
        Files.move(
    ...
    extract(
      ...
      buildNativeResourcesDirectory.resolve("darwin-aarch64").resolve("libWGPU.a"),
    )
    extract(
        "lib/libwgpu_native.dylib",
        jvmLibResourcesDirectory.resolve("darwin-aarch64").resolve("libWGPU.dylib"),
      )
      .doLast {
  ...
  download("wgpu-macos-x86_64-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("darwin-x64").resolve("libWGPU.a"),
    )
    extract(
        "lib/libwgpu_native.dylib",
        jvmLibResourcesDirectory.resolve("darwin-x86-64").resolve("libWGPU.dylib"),
      )
      .doLast {
  ...
  download("wgpu-windows-x86_64-gnu-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("windows-x64").resolve("wgpu.a"),
    )
  }
  download("wgpu-windows-x86_64-msvc-release.zip") {
    extract(
      "lib/wgpu_native.lib",
      buildNativeResourcesDirectory.resolve("windows-x64").resolve("wgpu.lib"),
    )
    extract(
        "lib/wgpu_native.dll",
        jvmLibResourcesDirectory.resolve("win32-x86-64").resolve("WGPU.dll"),
      )
      .doLast {
  ...
  download("wgpu-linux-x86_64-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("linux-x64").resolve("libWGPU.a"),
    )
    extract(
        "lib/libwgpu_native.so",
        jvmLibResourcesDirectory.resolve("linux-x86-64").resolve("libWGPU.so"),
      )
      .doLast {
  ...
  download("wgpu-linux-aarch64-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("linux-aarch64").resolve("libWGPU.a"),
    )
    extract(
        "lib/libwgpu_native.so",
        jvmLibResourcesDirectory.resolve("linux-aarch64").resolve("libWGPU.so"),
      )
      .doLast {
  ...
  download("wgpu-ios-x86_64-simulator-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("ios-simulator-x64").resolve("libWGPU.a"),
    )
  }
  download("wgpu-ios-aarch64-simulator-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("ios-simulator-aarch64").resolve("libWGPU.a"),
    )
  }
  download("wgpu-ios-aarch64-release.zip") {
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("ios-aarch64").resolve("libWGPU.a"),
    )
  }
  ...
  download("wgpu-android-x86_64-release.zip") {
    extract(
        "lib/libwgpu_native.so",
        buildNativeResourcesDirectory.resolve("libs").resolve("x86_64").resolve("libwgpu4k.so"),
      )
      .doLast {
        Files.move(
          buildNativeResourcesDirectory
    ...
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("android-x64").resolve("libWGPU.a"),
    )
  }
  download("wgpu-android-aarch64-release.zip") {
    extract(
        "lib/libwgpu_native.so",
        buildNativeResourcesDirectory.resolve("libs").resolve("arm64-v8a").resolve("libwgpu4k.so"),
      )
      .doLast {
        Files.move(
          buildNativeResourcesDirectory
    ...
    extract(
      "lib/libwgpu_native.a",
      buildNativeResourcesDirectory.resolve("android-aarch64").resolve("libWGPU.a"),
    )
  }
}
...
tasks.withType(CInteropProcess::class.java).configureEach { dependsOn("fetch-native-dependencies") }
...
tasks.register<Task>("generateDocs") {
  group = "documentation"
  description =
    "Generates the documentation in HTML and Markdown formats, then copies the files into the 'doc' folder."
