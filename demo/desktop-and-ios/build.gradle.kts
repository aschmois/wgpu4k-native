...
kotlin {
  ...
  if (nativeTarget != null) {
    with(nativeTarget) { binaries { executable { entryPoint = "io.ygdrasil.wgpu.main" } } }
  }
  ...
  sourceSets {
    commonMain { dependencies { implementation(projects.demo.common) } }

    val desktopMain by creating { dependencies { implementation(libs.ygdrasil.glfw.native) } }
...
tasks.register<JavaExec>("runJvm") {
  ...
  jvmArgs(
    if (Platform.os == Os.MacOs) {
      ...
      listOf("--add-opens=java.base/java.lang=ALL-UNNAMED", "--enable-native-access=ALL-UNNAMED")
    }
  )
